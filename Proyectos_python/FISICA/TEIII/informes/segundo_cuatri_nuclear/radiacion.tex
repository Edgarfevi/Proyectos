...
La instrumentación utilizada para el estudio de la radiación ionizante comprende: % [cite: 520]

\begin{itemize}
    \item \textbf{Tubo Geiger-Müller:} Detector cilíndrico de ionización con un radio de 1.43 cm y una longitud de 5.7 cm, provisto de una ventana de entrada delicada para partículas de bajo alcance. % [cite: 508, 515, 521]
    \item \textbf{Contador digital:} Unidad SPECTEC conectada a PC que permite el control de la alta tensión (HV) y el registro de cuentas por intervalos programables. % [cite: 518, 522, 531]
    \item \textbf{Banco de muestras:} Soporte con niveles o "escalones" situados a distancias fijas (separación de 1 cm entre ellos) para el estudio de la dependencia geométrica. % [cite: 634, 640]
    \item \textbf{Juego de absorbentes:} Láminas de plomo y aluminio de diferentes espesores y densidades superficiales para los ensayos de absorción de partículas $\beta$. % [cite: 654, 656]
    \item \textbf{Fuentes radiactivas:} Conjunto de emisores que incluye $^{90}\text{Sr}$ ($\beta$), $^{60}\text{Co}$ ($\gamma$), $^{238}\text{Pu}$ ($\alpha$) y $^{204}\text{Tl}$ ($\beta$). % [cite: 534, 535, 631]
\end{itemize}
