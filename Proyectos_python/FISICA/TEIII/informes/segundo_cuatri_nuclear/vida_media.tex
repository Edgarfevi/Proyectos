...
El sistema de detección y análisis espectroscópico se compone de los siguientes elementos: % [cite: 113]

\begin{itemize}
    \item \textbf{Detector de centelleo:} Cristal de NaI(Tl) de $38.1 \times 50.8$ mm acoplado a un tubo fotomultiplicador (PMT) y una unidad de alta tensión (HV) que opera a aproximadamente 735 V. % [cite: 114, 156]
    \item \textbf{Analizador Multicanal (MCA):} Dispositivo capaz de procesar hasta 2000 pulsos/s para la discriminación de energías. % [cite: 117]
    \item \textbf{Generador de isótopos:} Kit de elución que contiene una fuente de $^{137}\text{Cs}$ de 370 kBq, del cual se extrae el $^{137\text{m}}\text{Ba}$ mediante una solución salina acidulada (0.9\% NaCl en 0.04M HCl). % [cite: 41, 42, 120]
    \item \textbf{Software de análisis:} Programa PHYWE Measure configurado para la grabación de espectros y medidas de integración temporal. % [cite: 118, 158, 206]
\end{itemize}
