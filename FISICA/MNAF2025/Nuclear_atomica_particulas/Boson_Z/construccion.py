...
class construccion_individually:
    ...
    def _selection(self):
        ...
        **Formato de archivo esperado** (.dat):
        ...
        if self.path.match("**/*_ee_*.dat"):
            ...
            raise ValueError(
                f"Archivo '{self.path.name}' no contiene '_ee_' ni '_mumu_' en el nombre."
            )
    ...
    def apply_mass_fit(self, hist, fit_type="gaus"):
        ...
        **Workflow:**
        ...
        if fit_type == "gaus":
            ...
            func = rt.TF1(
                f"fit_bw_{hist.GetName()}", "[0]*TMath::BreitWigner(x, [1], [2])", fit_min, fit_max
            )
        ...
        ndf = func.GetNDF()
        mass_mean = func.GetParameter(1)
        mass_mean_err = func.GetParError(1)

        if ndf > 0:
            chi2_reducido = chi2 / ndf
            nombre_particulas = f"{self.p1_name}-{self.p2_name}"
            print(
                f"Ajuste [{fit_type.upper():<7}] en {nombre_particulas:<18} | Chi2/NDF: {chi2:>7.2f} / {ndf:<3} = {chi2_reducido:.3f}"
            )
        else:
            print(
                f"Ajuste [{fit_type.upper():<7}] en {self.p1_name}-{self.p2_name} | ERROR: NDF es 0."
            )

        print(
            f"Masa invariante ajustada [{fit_type.upper():<7}] en {self.p1_name}-{self.p2_name:<18} | "
            f"m_ll = {mass_mean:.3f} ± {mass_mean_err:.3f} GeV/c^2"
        )
    ...
    def plotter(self):
        ...
        **Physics:**
        ...
        def _autofit_single_hist(
            hist, y_padding=1.2, include_errors=False, anchor_y_one=False, central_fraction=0.995
        ):
        ...
        canvas_comparasion.Divide(
            2, 4
        )  # 2 columnas (pT, η) × 4 filas (p1 individual, p2 individual, overlay, ratio)
        ...
        hist_ratio_pt.Divide(
            hist_ratio_pt_den
        )  # Calcula ratio bin-by-bin con propagación de errores
        ...
        canvas_comparasion.SaveAs(
            str.format(
                f"{self.BASE_DIR}/img/{self.type}_comparasion_pt_eta_{self.p1_name}_{self.p2_name}.png"
            )
        )
        ...
        canvas.SaveAs(
            str.format(
                f"{self.BASE_DIR}/img/{self.type}_invariant_mass_Z_{self.p1_name}_{self.p2_name}.png"
            )
        )
