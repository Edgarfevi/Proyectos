...
class total_comparation:
    ...
    def plot_invariant_mass_overlay_channels(self, normalize=False):
        ...
        for pad_idx, type in enumerate(sample_types, start=1):
            ...
            if len(type_hists) > 1:
                ...
                self._autofit_mass_pair(
                    first_hist, second_hist, central_fraction=0.995, y_padding=1.25
                )
            else:
        ...
        canvas.Draw()
        output_name = (
            "invariant_mass_overlay_channels_by_type_norm.png"
            if normalize
            else "invariant_mass_overlay_channels_by_type.png"
        )
    ...
    def plot_comparative_figures_individually(self):
            ...
            for p1_name, p2_name in [("electron", "positron"), ("muon", "anti-muon")]:
                for type in ["gen", "reco", "data"]:
                    ...
                    if df is not None and plot_type == "pt":
                        ...
                        hist_canal = rt.TH1F(
                            f"pt_canal_{type}_{p1_name}_{p2_name}",
                            f" ({type});m_{{ll}} (GeV/c^{{2}});Eventos",
                            150,
                            0,
                            500,
                        ...
                        hist_canal = rt.TH1F(
                            f"eta_canal_{type}_{p1_name}_{p2_name}",
                            f" ({type});m_{{ll}} (GeV/c^{{2}});Eventos",
                            100,
                            -20,
                            20,
        ...
        canvas_ratios = rt.TCanvas(
            "canvas_comparative", "Comparacion de gen, reco y data", 1600, 1200
        )
            ...
            for current_particle in [p1_name, p2_name]:
                ...
                for num_type, den_type, ratio_title in ratios_to_plot:
                    for plot_type in ["pt", "eta"]:
                        ...
                        h_num = getattr(
                            self, f"hist_{num_type}_{current_particle}_{plot_type}", None
                        )
                        h_den = getattr(
                            self, f"hist_{den_type}_{current_particle}_{plot_type}", None
                        )

                        if h_num and h_den and h_den.Integral() > 0:
                            # Clonar histogramas originales
                            h_num_norm = h_num.Clone(
                                f"norm_{num_type}_{current_particle}_{plot_type}"
                            )
                            h_den_norm = h_den.Clone(
                                f"norm_{den_type}_{current_particle}_{plot_type}"
                            )
                            ...
                            h_ratio = h_num_norm.Clone(
                                f"ratio_{num_type}_{den_type}_{current_particle}_{plot_type}"
                            )
                            ...
                            h_ratio.SetTitle(
                                f"{ratio_title} de {plot_type} ({current_particle});{variable_title};{ratio_title}"
                            )
                            ...
                            setattr(
                                self,
                                f"ratio_obj_{num_type}_{den_type}_{current_particle}_{plot_type}",
                                h_ratio,
                            )
                            setattr(
                                self,
                                f"ratio_line_{num_type}_{den_type}_{current_particle}_{plot_type}",
                                line_ref,
                            )
                        else:
                            print(
                                f"Faltan datos o division por cero para {num_type}/{den_type} de {plot_type} ({current_particle})"
                            )
                ...
                for num_type, den_type, ratio_title in ratios_to_plot:
                    for plot_type in ["pt", "eta"]:
                        ...
                        if h_num and h_den and h_num.Integral() > 0 and h_den.Integral() > 0:
                            # Clonar para overlay
                            h_num_overlay = h_num.Clone(
                                f"overlay_{num_type}_{current_particle}_{plot_type}"
                            )
                            h_den_overlay = h_den.Clone(
                                f"overlay_{den_type}_{current_particle}_{plot_type}"
                            )
                            ...
                            self._autofit_overlay_pair(
                                h_num_overlay, h_den_overlay, central_fraction=0.995, y_padding=1.25
                            )
                            ...
                            setattr(
                                self,
                                f"overlay_obj_{num_type}_{den_type}_{current_particle}_{plot_type}",
                                h_num_overlay,
                            )
                            setattr(
                                self,
                                f"overlay_obj_den_{num_type}_{den_type}_{current_particle}_{plot_type}",
                                h_den_overlay,
                            )
                            setattr(
                                self,
                                f"overlay_legend_{num_type}_{den_type}_{current_particle}_{plot_type}",
                                legend_overlay,
                            )
                        else:
                            print(
                                f"Faltan datos o division por cero para overlay {num_type}/{den_type} de {plot_type} ({current_particle})"
                            )
                ...
                canvas_ratios.Draw()
                canvas_ratios.SaveAs(
                    str(self.BASE_DIR / "img" / f"comparative_ratios_{current_particle}.png")
                )
                setattr(self, f"canvas_ratios_{current_particle}", canvas_ratios)

                canvas_overlay.Draw()
                canvas_overlay.SaveAs(
                    str(self.BASE_DIR / "img" / f"comparative_overlay_{current_particle}.png")
                )
    ...
    def free_memory(self):
        ...
        if (
            hasattr(self, "canvas_invariant_mass_overlay_channels")
            and self.canvas_invariant_mass_overlay_channels
        ):
            self.canvas_invariant_mass_overlay_channels.Close()
            self.canvas_invariant_mass_overlay_channels = None

        if (
            hasattr(self, "canvas_invariant_mass_overlay_types_by_channel")
            and self.canvas_invariant_mass_overlay_types_by_channel
        ):
